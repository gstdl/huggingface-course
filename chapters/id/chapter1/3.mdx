# Apa yang bisa dilakukan oleh Transformers?
...
Di sub-bab ini, anda akan mempelajari mengenai model-model Transformer. Termasuk mengenai apa yang bisa dilakukan dengan Transformer serta menggunakan fungsi `pipeline()` dari modul 🤗 Transformers.

<Tip>
Tombol <em>Open in Colab</em> di kanan atas akan membuka Google Colab notebook di tab baru. _Notebook_ ini mengandung semua sampel kode untuk sub-bab ini. Tombol ini akan muncul di setiap sub-bab yang memiliki sampel kode.
Jika anda ingin menjalankan sampel kode di komputer anda, silahkan buka sub-bab <a href="/course/chapter0">setup</a>.
</Tip>

## Transformers ada dimana-mana!

Seperti yang disampaikan pada sub-bab sebelumnya, applikasi dari model-model Transformer dapat ditemui pada semua permasalahan NLP. Daftar perusahaan dan organisasi yang menggunakan Hugging Face dan model-model Transformer dapat dilihat pada gambar dibawah. Bukan hanya sebagai pengguna, perusahaan-perusahaan tersebut juga berkontribusi pada komunitas Hugging Face dengan membagikan model-model yang sudah mereka buat!

<img src="https://huggingface.co/datasets/huggingface-course/documentation-images/resolve/main/en/chapter1/companies.PNG" alt="Daftar perusahaan yang menggunkan Hugging Face" width="100%">

[Modul 🤗 Transformers](https://github.com/huggingface/transformers) menyediakan fungsi-fungsi untuk membuat model serta membagikan model tersebut ke seluruh dunia. [_Model Hub_](https://huggingface.co/models) adalah rumah dari ribuan _pretrained models_ (model-model yang sudah dilatih terlebih dahulu) dan model-model tersebut dapat diunduh dan digunakan oleh siapa saja. Anda juga dapat menunggah model buatan anda ke _Model Hub_!

<Tip>
⚠️ Bukan hanya model-model Transformer, semua jenis model atau dataset dapat diunggah dan diunduh oleh siapa saja di _Hugging Face Hub_! Silahkan buat akun di <a href="https://huggingface.co/join">huggingface.co</a> untuk menggunkan semua fitur yang disediakan oleh tim Hugging Face!
</Tip>

Sebelum mempelajari alur pembuatan model Transformer, mari mempelajari beberapa contoh penggunaan model Transformer untuk menyelesaikan beberapa tantangan NLP yang cukup menarik.

## Menggunakan `pipeline`

<Youtube id="tiZFewofSLM" />

Objek paling penting dari modul 🤗 Transformers adalah fungsi `pipeline()`. Ia berperan untuk menangani semua tahap-tahap penting dalam _machine learning_ seperti _preprocessing_ dan _postprocessing_ yang membuat model anda mampu memproses segala macam input teks dan memberikan jawaban yang sesuai:
...
Bukan hanya satu kalimat, tapi `pipeline()` mampu memproses beberapa kalimat sekaligus!
